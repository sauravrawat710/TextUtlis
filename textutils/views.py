...
def analyze(request):
    ...
    if (charcounter == "on"):
        wordcount = len(djtext)
        Counter = f"Total Character = {wordcount}"
        params = {'purpose': 'Character Counter',
                  'analyzed_text': analyzed, 'charcounter': Counter}

    if (removepunc != "on" and fullcaps != "on" and newlineremover != "on" and extraspaceremover != "on" and charcounter != "on"):
        return HttpResponse("<h1>Please Select atleast one opration.</h1>")
